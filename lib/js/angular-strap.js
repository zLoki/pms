    ...
    this.$get = ["$window", "$document", "$rootScope", "$sce", "$locale", "dateFilter", "datepickerViews", "$tooltip", function($window, $document, $rootScope, $sce, $locale, dateFilter, datepickerViews, $tooltip) {
      ...
      function DatepickerFactory(element, controller, config) {
        ...
        $datepicker.select = function(date, keep) {
          ...
          // VladimirSh 21.10.2014: Fix for Datepicker to work with UTC TZ instead of local
          if(!angular.isDate(controller.$dateValue)) controller.$dateValue = new Date(date.getTime() - date.getTimezoneOffset() * 60000);
          if(!scope.$mode || keep) {
  ...
  .directive('bsDatepicker', ["$window", "$parse", "$q", "$locale", "dateFilter", "$datepicker", "$dateParser", "$timeout", function($window, $parse, $q, $locale, dateFilter, $datepicker, $dateParser, $timeout) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        // VladimirSh 21.10.2014: Fix for Datepicker to work with UTC TZ instead of local
        var dateParser = $dateParser({format: "dd MMM yyyy HH:mm", lang: options.lang, strict: options.strictFormat});
        ...
        controller.$parsers.unshift(function(viewValue) {
          ...
          // VladimirSh 11.11.2014: Fix for Datepicker to work with UTC TZ instead of local
          if(!dateParser.parse(viewValue, controller.$dateValue, "dd MMM yyyy")) {
          ...
          var parsedDate = dateParser.parse(new Date(viewValue.getTime() - viewValue.getTimezoneOffset() * 60000), controller.$dateValue);
        ...
        controller.$render = function() {
          ...
          element.val(!controller.$dateValue || isNaN(controller.$dateValue.getTime()) ? '' : dateFilter(new Date(controller.$dateValue.getUTCFullYear(), controller.$dateValue.getUTCMonth(), controller.$dateValue.getUTCDate(),  controller.$dateValue.getUTCHours(), controller.$dateValue.getUTCMinutes(), controller.$dateValue.getUTCSeconds()), options.dateFormat));
        };
      ...
      return function(picker) {
        ...
        var views = [{
            ...
            isSelected: function(date) {
              if (!picker.$date) return false;
              // VladimirSh 21.10.2014: Fix for Datepicker to work with UTC TZ instead of local
              //return picker.$date && date.getFullYear() === picker.$date.getFullYear() && date.getMonth() === picker.$date.getMonth() && date.getDate() === picker.$date.getDate();
              var pickerDate = new Date(picker.$date.getUTCFullYear(), picker.$date.getUTCMonth(), picker.$date.getUTCDate(),  picker.$date.getUTCHours(), picker.$date.getUTCMinutes());
              return date.getFullYear() === pickerDate.getFullYear() && date.getMonth() === pickerDate.getMonth() && date.getDate() === pickerDate.getDate();
            },
